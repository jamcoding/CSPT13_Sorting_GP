...
def insertion_sort(list_to_sort):
    ...
    # for all the other items (on the right of the first index)
    # range based starting at the index 1
    for i in range(1, len(list_to_sort)):
        # put the current number in to a temporary variable
        temp = list_to_sort[i]

        # keep track of our current index as j
        j = i

        # keep looking left, until we find where our temporary number belongs
        # while j is greater than 0 (we are not past the start of the indeces)
        ...
        # when left is smaller than temporary, or we are at zero, put the number at that spot
        list_to_sort[j] = temp
